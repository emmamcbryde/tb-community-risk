...
|---|---|---|
| already ported | APY config/defaults, validation, data loading, age distribution, regimen handling, calibration, cohort primitives, single-cohort simulation, replicate runner, summary rows, minimal results bundle, natural-history dynamic comparison support, parity helpers, distributional validation runner | Python-native implementation exists under `engine/apy/` with tests and compact MATLAB fixture validation. |
