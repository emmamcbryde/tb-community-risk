...
The APY v9 model is a MATLAB-based agent-based model for LTBI screening and preventive treatment scenarios. It is exposed through a Streamlit workflow in this branch using a thin Python adapter around the MATLAB backend.
...
│   ├── 2_Run_Model.py            # Run MATLAB APY v9 backend
...
## MATLAB requirements for the APY v9 workflow

The APY v9 workflow in the Streamlit app is MATLAB-backed. To run APY v9 scenarios from Streamlit, the local environment needs:
...
The Python adapter starts MATLAB and adds the `abm/` folder to the MATLAB path when APY backend functions are called.

If MATLAB is not available, the app may still start, but APY pages that call the MATLAB backend will not be able to run scenarios. The Python dynamic/static components should be developed so they can remain runnable without MATLAB.
...
Important deployment note: Streamlit Cloud should not be assumed to have MATLAB installed. MATLAB-backed APY execution is therefore primarily a local workflow unless a separate MATLAB-capable backend is configured. For hosted deployment, keep Python-only pages and any CSV/output-reading workflows separated from direct MATLAB execution.
