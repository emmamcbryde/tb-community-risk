...
st.markdown("Estimate tuberculosis risk based on community characteristics and interventions.")
...
model_mode = st.sidebar.radio("Choose model type:",
    ["Static incidence model", "Dynamic LTBI→TB model"]
)
...
age_method = st.sidebar.radio(
    ...
    ["Fetch from World Bank/OWID", "Use local CSV", "Default global", "Upload custom CSV"]
)
...
if st.sidebar.button("Simulate Community"):
    ...
    inc_hist = { -k: user_incidence for k in range(0, 21) }
    ...
    try:
        ...
        hover = alt.selection_single(fields=["Year"], nearest=True, on="mouseover", empty="none")

        chart = alt.Chart(df).mark_line().encode(
            x="Year:Q",
            y="Incidence_per_100k:Q",
            color="Scenario:N"
        )

        points = chart.mark_circle(size=120).encode(
        ...
        ltbi_df = pd.DataFrame({"Age": ages, "LTBI Prevalence": [ltbi_prev[a] for a in ages]})
